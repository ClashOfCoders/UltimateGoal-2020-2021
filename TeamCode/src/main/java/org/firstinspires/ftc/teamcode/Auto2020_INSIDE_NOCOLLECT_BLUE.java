...
@Autonomous(name= "Blue No Collect Inside")
public class Auto2020_INSIDE_NOCOLLECT_BLUE extends LinearOpMode {
    //camera variables
    ...
    public void runOpMode() {
        ...
        Trajectory initialShot = drive.trajectoryBuilder(drive.getPoseEstimate())
                .lineToLinearHeading(new Pose2d(-6.0,12.0,Math.atan2((36 - 12), (74 + 6))+shootAngleOffsetRadians+Math.toRadians(-3)))
                .build();
        ...
        Trajectory dumpWobble1 = drive.trajectoryBuilder(initialShot.end())
                .splineToSplineHeading(new Pose2d(36.0, 18.0, Math.toRadians(90.0)), 0.0)
                .addSpatialMarker(new Vector2d(12.0,18.0), new MarkerCallback() {
                    @Override
                    public void onMarkerReached() {
                        robot.wobbleMotor.setTargetPosition(wobblePickUp);
                    }
        ...
        robot.shooter.setVelocity(1575);
        robot.shooter2.setVelocity(1575);
        sleep(200);
        if(Ring != null) {
            if (Ring.center.x < 85) {
                Ring = null;
        ...
        switch(ringState)
        {
            case 0:
                ...
                while(opModeIsActive() && robot.shooter.getVelocity()<(1575 - 21) && robot.shooter.getVelocity()>(1575 + 21))
                {
                    robot.shooter.setVelocity(1575);
                    robot.shooter2.setVelocity(1575);
                }
    ...
    static class RingDetectionPipeline extends OpenCvPipeline
    {
        ...
        void analyzeContour(MatOfPoint contour, Mat input)
        {
            ...
            if (rotatedRectFitToContour.size.area() >= 200 && rotatedRectFitToContour.center.x > 90 && rotatedRectFitToContour.center.x < 150 && rotatedRectFitToContour.center.y > 100 && rotatedRectFitToContour.center.y < 300) {
